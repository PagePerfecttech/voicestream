...
describe('Channel Data Models and Validation - Core Functions', () => {
  describe('Channel Configuration Validation', () => {
    ...
    test('should validate fallback video URL', () => {
      const config: CreateChannelRequest = {
        name: 'Test Channel',
        resolution: 'HD',
        bitrate: 2000,
        fallbackVideo: 'not-a-valid-url',
      };

      const errors = validateChannelConfiguration(config);
      expect(errors).toContain('Fallback video must be a valid URL');
    });
  });
